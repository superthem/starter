...
# 生成的项目目录名：有父项目则为 父项目名-项目名-application，否则为 项目名-application
if [[ -n "$parent_name" ]]; then
    output_root="${parent_name}-${project_name}-application"
else
    output_root="${project_name}-application"
fi
...
clone_url="https://github.com/superthem/example-application.git"
...
# 遍历所有目录，重命名目录
find "./${output_root}" -depth -type d -name "*exampleaaa*" | while read -r dir; do
    new_dir=$(echo "$dir" | sed "s/exampleaaa/${company_name}/g")
    mv "$dir" "$new_dir"
done

find "./${output_root}" -depth -type d -name "*exampleeee*" | while read -r dir; do
